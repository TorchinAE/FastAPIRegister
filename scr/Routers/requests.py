...
@req_router.get("/search/duplicates")
async def search_duplicates(
    project_stamp: str | None = Query(None),
    ...
    conditions = []
    if project_stamp and len(project_stamp.strip()) >= 2:
        conditions.append(ReqModel.project_stamp == project_stamp.strip())
    if description and len(description.strip()) >= 3:
        conditions.append(ReqModel.description == description.strip())
    ...
    stmt = (
        select(ReqModel)
        .options(selectinload(ReqModel.equipment))
        .where(or_(*conditions))
        .order_by(ReqModel.id.desc())
        .limit(20)
    )
    if exclude_id:
        stmt = stmt.where(ReqModel.id != exclude_id)
