...
async def users_detail_page(
    ...
    user_requests, _ = await crud_requests.get_requests(session, manager_id=user_id, per_page=100)
    return templates.TemplateResponse(
        "users/detail.html",
        {"request": request, "user": user, "target": target, "user_requests": user_requests, "back_to": back_to, "active_page": "users"},
    )
...
async def equipment_detail_page(
    ...
    eq_requests, _ = await crud_requests.get_requests(session, equipment_id=eq_id, per_page=100)
    return templates.TemplateResponse(
        "equipment/detail.html",
        {"request": request, "user": user, "eq": eq, "eq_requests": eq_requests, "back_to": back_to, "active_page": "equipment"},
    )
