...
async def get_dirs(
    ...
    stmt = select(Directors).options(selectinload(Directors.position), selectinload(Directors.organizations)).order_by(Directors.name)
...
async def get_dir_to_id(session: AsyncSession, dir_id: int) -> Directors | None:
    stmt = select(Directors).options(selectinload(Directors.position), selectinload(Directors.organizations)).where(Directors.id == dir_id)
    result: Result = await session.execute(stmt)
