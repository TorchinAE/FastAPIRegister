...
async def add_director(
    name: str, short_name: str, email: str, phone: str, post_id: int, petition_id: int
):
    async with async_session() as session:
        try:
            ...
            if result.scalar_one_or_none():
                raise HTTPException(
                    status_code=409, detail="Директор с таким именем уже существует."
                )
            director = Directors(
                name=name,
            ...
            stmt_full = (
                select(Directors)
                .options(selectinload(Directors.post), selectinload(Directors.petition))
            ...
            logging.error(f"Ошибка при сохранении директора {name} {email}: {e}")
            raise HTTPException(
                status_code=500, detail="Ошибка данных при сохранении директора."
            )
    ...
    async with async_session() as session:
        try:
            ...
            return HTTPException(
                status_code=500, detail="Ошибка при сохранении директоров"
            )


async def get_dirs() -> DirectorListResponse | list:
    async with async_session() as session:
        stmt = select(Directors).options(
            selectinload(Directors.post), selectinload(Directors.petition)
        )
        result = await session.execute(stmt)
