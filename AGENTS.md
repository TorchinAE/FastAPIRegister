...
- **Table creation**: lifespan in `main.py` calls `Base.metadata.create_all` on every startup. Alembic is also configured for migrations — use `uv run alembic revision --autogenerate -m "desc"` then `uv run alembic upgrade head` for schema changes. Alembic's `env.py` uses the sync driver (strips `+aiosqlite`) and `render_as_batch=True` for SQLite ALTER TABLE support.
...
1. **AmbiguousForeignKeysError**: When adding models with FK to `managers.id`, SQLAlchemy gets confused because `BaseID.changed_by_id` also points to `managers.id`. Always specify `foreign_keys=[...]` explicitly on relationships that target `managers`.

2. **Eager loading required**: Directors' `position` relationship must be loaded eagerly (`selectinload`) in async context — lazy access raises `MissingGreenlet`.

3. **Positions field renamed**: `Positions.title` was renamed to `Positions.name` to match MVP spec.

4. **`.gitignore` excludes `pyproject.toml`** — this is unusual; the file is tracked but listed in gitignore.
...
- pytest `asyncio_mode = "auto"` is set in `pyproject.toml` — async test functions are auto-wrapped, `@pytest.mark.asyncio` is optional.
